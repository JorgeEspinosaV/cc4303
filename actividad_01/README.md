...
En esta actividad vamos a construir un proxy para filtrar contenido web.  
La actividad se divide en 2 partes para mantener el orden, sin embargo la **parte 1 es más corta** que la parte 2 y solo se entrega la versión final de la parte 2.

A continuación veremos algunos de los conceptos que necesitaremos saber para realizar esta actividad.  
Para esta actividad además necesitará del material provisto en el video: **HTTP** y el material de la sección **Mensajes HTTP**.

---

## Puntaje

**4.5 pts código + 1.5 pts informe**
...
|---------------|--------|
| El cliente recibe una respuesta | +0.5 |
| El proxy bloquea correctamente los sitios prohibidos | +1.0 |
| Se reemplazan correctamente las palabras según el json utilizado | +1.2 |
| Se modifican correctamente los headers | +0.5 |
| Se manejan correctamente mensajes más grandes que el buffer del socket | +1.3 |
...
- Un proxy es cualquier dispositivo intermedio entre un cliente y servidor, comúnmente utilizado para realizar las consultas a nombre del cliente y luego reenviárselas.
- Un proxy necesita ser capaz de recibir consultas como lo haría un servidor y luego (re)enviarlas como lo haría un cliente.
- Un ejemplo de uso de proxy es acceder desde sus casas a artículos científicos usando el proxy del DCC. Artículos a los que ustedes no tienen acceso, pero la Universidad sí. Esto funciona pues al utilizar el proxy del DCC para solicitar un artículo a una biblioteca a la cual la Universidad tiene acceso, el proxy del DCC reenvía su petición desde la red del DCC la cual es parte de la red de la Universidad. Luego desde la biblioteca ven que la Universidad está solicitando un artículo y como esta tiene acceso a la biblioteca, le entregan el artículo sin problemas. Finalmente el artículo es recibido por el proxy del DCC y quien reenvía el artículo a ustedes en sus casas.

### Antes de comenzar

#### Maquina virtual

A la hora de ejecutar nuestro proxy, lo echaremos a correr en una máquina virtual y anclaremos el mismo a la IP de dicha máquina.

#### Tipo de socket usado por HTTP

El protocolo HTTP utiliza sockets orientados a conexión, por lo que los clientes deben invocar a la función `connect` para conectarse al servidor, mientras que los servidores deben llamar a `accept` para aceptar peticiones de clientes.
...
> **Nota:** No es necesario entregarla, es para comenzar en orden.
...
1. **Configuración de máquina virtual**: Para esta actividad ejecutaremos los códigos a desarrollar en una máquina virtual y asociaremos los respectivos sockets a la IP de dicha máquina a la cuál nos referiremos como `IP_VM` por IP Virtual Machine. Respecto a la máquina virtual, puede ejecutar una máquina virtual tal como lo hizo en el curso anterior de Sistemas Operativos. Como primer paso cree su máquina virtual en caso de no tener una del curso anterior, y revise cuál es la IP de la misma, esta será su `IP_VM`.

2. **Parseo y creación de mensajes HTTP**: Como primer paso a nivel de código encárguese de que su servidor pueda leer, interpretar y crear mensajes HTTP. Para ello cree la función `parse_HTTP_message(http_message)` que tome un mensaje HTTP y lo transfiera a una estructura de datos que le permita acceder fácilmente a la información del mensaje. Además cree la función `create_HTTP_message` que sea capaz de tomar la estructura de datos entregada por `parse_HTTP` y la convierta en un mensaje HTTP. Para saber qué información debería extraer, revise el material sobre mensajes HTTP, evalúe qué debería extraer su función `parse` y anótelo en su informe.

3. **Test - Request del navegador**: Utilice su navegador para obtener un 'request' y use dicho request para probar que las funciones que acaba de programar funcionen como corresponde. Para obtener la request cree un socket servidor que escuche en `(IP_VM, 8000)` en su máquina virtual, e intente acceder a `http://IP_VM:8000` desde un navegador en su máquina local. Al hacer esto su socket recibirá la 'request' HTTP que está haciendo el navegador a su socket servidor. Para ver la request imprima lo que recibió su socket en pantalla usando `print` antes de invocar a `decode`. Utilice esta request para probar el correcto funcionamiento de sus funciones `parse_HTTP_message` y `create_HTTP_message`. (Note que en este punto su código no le responde nada al navegador, por lo que su navegador no va a mostrar nada).

4. **Crear respuesta HTTP**: Ahora, abra una consola (terminal) y utilice `curl` para obtener una 'response' o respuesta a una petición `GET`. Para ello ejecute el siguiente comando en consola (recuerde remover el '%'):

```bash
curl -i cc4303.bachmann.cl
```

Al ejecutar este comando, verá la respuesta a la request o petición `GET` hecha por `curl`. Note que en esta respuesta usted ve los saltos de línea sin mayor detalle, sin embargo cada salto de línea tiene la forma `"\r\n"`. Utilice esta response o respuesta HTTP como guía para crear su propia response a la petición que estaba recibiendo en el paso (1). Para ello deberá crear un pequeño HTML que pueda mostrarse en su navegador y sus headers deberán ser consistentes con el largo y tipo de contenido de su mensaje HTTP. Utilice la respuesta que acab de crear para responder la petición que le hizo el navegador en el paso (1).

5. **Test - Verificar respuesta**: Haga que su código (corriendo en su máquina virtual) le responda al navegador con la respuesta que acaba de crear y verifique que el navegador logra mostrarla. Luego utilice `curl` desde su máquina local como se muestra a continuación y verifique que `curl` también recibe una respuesta satisfactoria.

```bash
curl -i IP_VM:8000
```

6. **Agregar header personalizado**: Modifique su servidor para que al momento de responder le agregue el header `X-ElQuePregunta` con su nombre como valor. (Esto nos va a servir en la parte 2, no cambie el nombre del header).

7. **Test - Verificar header**: Pruebe que se agrega de forma correcta el header utilizando `curl` como se muestra a continuación:

```bash
curl IP_VM:[puerto_server_http] -i
```

8. **Archivos JSON**: Modifique su servidor para que pueda leer archivos JSON o archivos de configuración. Haga que el nombre y ubicación del archivo JSON pueda ser recibido como argumento al ejecutar su código. Estos archivos serán necesarios más adelante en la parte 2 para darle ciertas instrucciones a nuestro servidor. Por mientras úselo para dejar en una variable su nombre, así el servidor quedará con un usuario parametrizable. Puede encontrar un ejemplo de uso de JSON en la sección **Material e indicaciones para la actividad** (más abajo).

9. **Test - JSON con nombre**: Pruebe que su servidor puede tomar su nombre desde su archivo JSON y lo puede utilizar para añadir el header `X-ElQuePregunta`.

---

## Parte 2: Implementación del proxy

> **Nota:** Esta parte sí se entrega.
...
1. Bloquear tráfico hacia páginas no permitidas (como un control parental)
2. Reemplazar contenido inadecuado (reemplazo el string A con el string B)
...
1. **Proxy básico sin modificación**: Modifique su servidor para que sea un proxy que esté entre un cliente y un servidor, pero que no modifique el mensaje, esto es: recibe un requerimiento, se lo envía al servidor de forma íntegra, recibe la respuesta y se la envía al cliente sin modificarla. Para crear esta modificación a su código siga el diagrama que acaba de realizar. Note que para lograr esto necesitará poder comunicarse con el cliente y el servidor al mismo tiempo.

2. **Test - Transferencia de mensajes**: Use `curl` para ver que su proxy logra transferir mensajes de forma exitosa. Para ello verifique que pedir la página `example.com` con `curl` sin usar proxy, entrega lo mismo que al usar `curl` con su código como proxy. Para probar esto ejecute los siguientes comandos:
...
3. **Bloquear sitios prohibidos**: Modifique su proxy para que al recibir la URI (protocolo + dirección; por ejemplo `http://www.example.com`) del servidor, chequee si la dirección está bloqueada. De ser así devuelva el código de error 403 junto a un HTML que muestre una imagen almacenada localmente por usted, para ello puede utilizar la etiqueta `<img src=[file_path]>` (se sugiere usar gatos). Para saber qué direcciones están bloqueadas utilice el archivo JSON de la sección **Material e indicaciones para la actividad** que sale más abajo.

4. **Test - Sitios prohibidos**: Utilizando su proxy con `curl` intente acceder a una página prohibida y verifique que este retorna error 403 junto al código HTML que usted implementó. Verifique que al ingresar a páginas que no están prohibidas, como `cc4303.bachmann.cl`, su código sigue funcionando como antes.

5. **Agregar header a requests**: En caso de que la dirección del servidor final no sea una dirección prohibida, agregue a la request que va desde el proxy al servidor el header `X-ElQuePregunta` con su nombre.

6. **Test - Verificar header**: Utilizando `curl`, pruebe acceder al dominio `cc4303.bachmann.cl` a través de su proxy y verifique que el mensaje de bienvenida mostrado por la página cambia al pasar por su proxy, versus el mensaje mostrado al usar `curl` sin pasar por su proxy.

7. **Reemplazar contenido**: Configure su proxy para que reemplace contenido inadecuado. Para esto busque las palabras prohibidas (marcado como `forbidden_words` en el JSON mostrado en la sección **Material e indicaciones para la actividad** más abajo) y reemplace dichas palabras (reemplazo el string A con el string B).

8. **Test - Reemplazo de palabras**: Utilizando `curl` pruebe acceder a `cc4303.bachmann.cl/replace` a través de su proxy y verifique que las palabras son reemplazadas y que no hay errores en el contenido.

9. **Manejo de mensajes grandes**: Finalmente modifique su código para que pueda recibir mensajes utilizando sockets cuyo buffer de recepción sea más pequeño que el tamaño del mensaje a recibir. Para ello puede usar/modificar las funciones provistas en los códigos de ejemplo de la semana 1 (sockets). Para implementar esta parte deberá responder las siguientes preguntas:
   - ¿Cómo sé si llegó el mensaje completo?
   - ¿Qué pasa si los headers no caben en mi buffer?
   - ¿Cómo sé que el HEAD llegó completo?
   - ¿Y el BODY?

   Al documentar su código en el informe, responda explícitamente estas preguntas.

10. **Test - Buffer pequeño**: Pruebe que su proxy sigue funcionando cuando el tamaño del buffer de recepción es pequeño (ejemplo: `recv_buffer = 50`).
...
1. Verifique que ya **NO** puede ver el sitio `http://cc4303.bachmann.cl/secret`, si no que recibe un error 403.
2. Verifique que al acceder al sitio `http://cc4303.bachmann.cl/` el contenido de este se modifica según los cambios introducidos al área de headers.
3. Verifique que puede ver los sitios `http://cc4303.bachmann.cl/` y `http://cc4303.bachmann.cl/replace`, pero las palabras prohibidas han sido modificadas. Verifique que el texto mostrado sea el mismo solo que con las palabras prohibidas censuradas.
4. Pruebe que su proxy funciona en los siguientes casos:
   - El tamaño del buffer de recepción del socket es más pequeño que el tamaño del mensaje, pero más grande que el tamaño del área de headers.
   - El tamaño del buffer es más pequeño que el tamaño del área de headers, pero más grande que la start line.

Anote el resultado de sus pruebas en el informe.
...
En el siguiente ejemplo vemos cómo calcular la cantidad total de artículos de oficina en un inventario. Para ello tenemos el archivo `inventario.json`:
...
Usando el archivo `inventario.json` podemos calcular la cantidad total de artículos de oficina usando el siguiente código en Python:
...
¿Cómo modificarían este código para que entregue sólo la cantidad de lápices de colores?

### Probar proxy con curl

Puede realizar las pruebas iniciales de su proxy usando el comando `curl` con el flag `-x` para indicar el uso de proxy de la siguiente manera:

```bash
curl [domain] -x localhost:8000
```

### JSON páginas bloqueadas y palabras prohibidas

Las páginas bloqueadas y las palabras prohibidas puede encontrarlas en el siguiente JSON:
